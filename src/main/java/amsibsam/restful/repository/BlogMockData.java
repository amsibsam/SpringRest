package amsibsam.restful.repository;
...
public class BlogMockData {
    ...
    private static BlogMockData instance;
    ...
    public List<Blog> searchBlogs(String searchTerm) {
        ...
        for (Blog blog : blogs) {
            if (blog.getTitle().toLowerCase().contains(searchTerm.toLowerCase()) ||
                    blog.getContent().toLowerCase().contains(searchTerm.toLowerCase())) {
